...
#         rental_qs = RentalDeals.objects.filter(is_deleted='N').prefetch_related('submitted_by_user')
...
#         logger.debug(f"datatable_filter: POST data: {request.POST}")

#         rental_qs = RentalDeals.objects.filter(is_deleted='N').prefetch_related('submitted_by_user')
#         logger.debug(f"Initial RentalDeals count: {rental_qs.count()}")
...
#         logger.debug(f"Filtered RentalDeals count: {rental_qs.count()}")

#         sales_qs = SalesDeals.objects.filter(is_deleted='N').prefetch_related('submitted_by_user')
#         logger.debug(f"Initial SalesDeals count: {sales_qs.count()}")
...
#             sales_qs = sales_qs.filter(unit_details__icontains=unit_details)
#         if building_name:
#             sales_qs = sales_qs.filter(building_name__icontains=building_name)
#         if project_name:
#             sales_qs = sales_qs.filter(project_name__icontains=project_name)
#         if agent_name:
#             sales_qs = sales_qs.filter(
...
#         logger.debug(f"Filtered SalesDeals count: {sales_qs.count()}")

#         rental_data = RentalDealsSerializer(rental_qs, many=True).data
#         sales_data = SalesDealsSerializer(sales_qs, many=True).data

#         combined_data = rental_data + sales_data
#         logger.debug(f"Combined data length: {len(combined_data)}")
...
#                 return False
#             combined_data = list(filter(matches_search, combined_data))
#             logger.debug(f"After search filter, combined data length: {len(combined_data)}")
...
#         logger.debug(f"Agent dropdown users count: {users.count()}")
#         serializer = AgentDropdownSerializer(users, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)
...
#         logger.debug(f"performance_datatable_filter: POST data: {request.POST}")
...
#         if agent_name:
#             rental_qs = rental_qs.filter(submitted_by_user__name__icontains=agent_name)
#             logger.debug(f"After agent_name filter, RentalDeals count: {rental_qs.count()}")
#         if deal_type and deal_type != 'Rental':
#             rental_qs = rental_qs.none()
#             logger.debug("RentalDeals set to none due to deal_type filter")
#         if date_from:
#             rental_qs = rental_qs.filter(date__gte=date_from)
#             logger.debug(f"After date_from filter, RentalDeals count: {rental_qs.count()}")
#         if date_to:
#             rental_qs = rental_qs.filter(date__lte=date_to)
#             logger.debug(f"After date_to filter, RentalDeals count: {rental_qs.count()}")
...
#                 Q(agent_name2__icontains=agent_name)
#             )
#             logger.debug(f"After agent_name filter, SalesDeals count: {sales_qs.count()}")
#         if deal_type and deal_type != 'Sales':
#             sales_qs = sales_qs.none()
#             logger.debug("SalesDeals set to none due to deal_type filter")
#         if date_from:
#             sales_qs = sales_qs.filter(date__gte=date_from)
#             logger.debug(f"After date_from filter, SalesDeals count: {sales_qs.count()}")
#         if date_to:
#             sales_qs = sales_qs.filter(date__lte=date_to)
#             logger.debug(f"After date_to filter, SalesDeals count: {sales_qs.count()}")
...
#             for k, v in groups.items()
#         ]
#         logger.debug(f"Group list length: {len(group_list)}")
...
#         group_list = list(filter(matches_filters, group_list))
#         logger.debug(f"Filtered group list length: {len(group_list)}")
