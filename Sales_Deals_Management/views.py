...
from .serializers import (
    SalesDealSerializer,
    SalesDealSerializerForDraft,
    SalesDealSerializerFordatafilter,
    filterSerializer,
    AgentDropdownSerializer,
)
from django.db.models import Q
from django.contrib.auth.decorators import login_required, permission_required
from Rental_Deal.Utilities import (
    upload_file_to_full_s3_url,
    s3_file_exists,
)
import boto3
...
def _get_legacy_receipt_ids_from_source(source_data):
    ...
    for field in [
        ...
        value = (
            ...
            else getattr(source_data, field, None)
        )
...
def _build_receipts_list_from_legacy_fields(source_data):
    """
    Resolve the 5 fixed receipt_id.. receipt_id5 fields on source_data into
    the same {id, receipt_number} payload shape used for the dynamic
    receipts_list, so both mechanisms can be combined into one
    _sync_receipt_statuses call.
    """
    receipts_payload = []
    seen_ids = set()
...
def clone_deal_with_new_reference(old_deal_id, new_reference):
    logger.info(
        f"Cloning deal with ID: {old_deal_id} to new reference: {new_reference}"
    )
    """
...
class SalesDealViewSet(viewsets.ModelViewSet):
    ...
    def delete_sales(self, request, pk=None):
        ...
        dynamic_receipt_ids = [
            item["id"]
            for item in _parse_receipts_list(getattr(sales, "receipts_list", ""))
            if item.get("id")
        ]
        ...
        if all_receipt_ids:
            Receipts.objects.filter(
                id__in=all_receipt_ids, deal_refer_no=sales.reference_number
            ).update(status="Unused", deal_refer_no="")

        SalesDeals.objects.filter(pk=pk).update(is_deleted="Y")
        SalesDeals.objects.filter(pk=pk).update(
            reference_number=sales.reference_number + "D"
        )
    ...
    def clone_deal_with_new_ref(self, request, pk=None):
        logger.info(
            f"Received request to clone deal with ID: {pk} by referencenumber: {request.data.get('new_reference')}"
        )
        """
    ...
    def view_sales_deal(self, request, pk=None):
        ...
        if sales_deal.receipt_no == "Null" or sales_deal.receipt_no == "":
            ...
            sales_deal.receipt_no == "No Commission"
            or sales_deal.receipt_no == "No Commision"
        ):
            ...
            # Case 3: Receipt ID
            recipt = Receipts.objects.filter(
                receipt_number=sales_deal.receipt_no
            ).first()
            if recipt:
        ...
        if sales_deal.receipts_list:
            ...
            for item in parsed_list:
                receipts_list_view.append(
                    {"id": item.get("id"), "receipt_number": item.get("receipt_number")}
                )

        return render(
            request,
            "home/viewsalesdeal.html",
            {
    ...
    def create_sale_deal(self, request):
        ...
        dynamic_receipts = [
            r for r in parsed_receipts if r["id"] not in legacy_receipt_ids
        ]
        ...
        combined_receipts_payload = (
            _build_receipts_list_from_legacy_fields(mutable_data) + dynamic_receipts
        )
        ...
        if mutable_data["form_status"] == "Complete":
            mutable_data["submitted_date"] = (
                datetime.date.today()
            )  # changes htes data  to teh submitted date in the froet end
    ...
    def update_sales_deal(self, request, pk=None):
        ...
        if request.method == "GET":
            pk = pk
            sales_deal = get_object_or_404(SalesDeals, pk=pk)
            serializer = SalesDealSerializer(
                sales_deal, partial=True, context={"request": request}
            )
            return Response(serializer.data, status=status.HTTP_200_OK)
            ...
            dynamic_receipts = [
                r for r in current_parsed if r["id"] not in legacy_receipt_ids
            ]
            ...
            serializer = self.get_serializer(
                sales_deal, data=mutable_data, partial=True
            )
    ...
    def datatable_filter(self, request):
        ...
        queryset = (
            SalesDeals.objects.filter(is_deleted="N", account_id=account_id)
        ...
        filter_fields = [
            ...
            "builduing_name",
            "seller_source",
            "selller_mobileis_approved_rejected",
            "project_name",
            "seller_name",
            "buyer_name",
            "buyer_mobile",
        ...
        if type_filter:
            ...
            if type_filter == "pending":
                if user.has_perm("core.view_pending_sales_deals"):
                    ...
                    return Response(
                        {
                            "detail": "You do not have permission: view_pending_sales_deals"
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )
            ...
            # ---------------- Approved ----------------
                ...
                if user.has_perm("core.view_approved_sales_deals"):
                    ...
                    return Response(
                        {
                            "detail": "You do not have permission: view_approved_sales_deals"
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )
            ...
            # ---------------- Rejected ----------------
                ...
                if user.has_perm("core.view_rejected_sales_deals"):
                    if account_id and (role in ["Manager"] or user.is_superuser):
                        queryset = queryset.filter(
                            manager_approved_rejected="R", form_status="Complete"
                        )
                    elif role not in ["Agent"]:
                        queryset = queryset.filter(
                    ...
                    return Response(
                        {
                            "detail": "You do not have permission: view_rejected_sales_deals"
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )
            ...
            # ---------------- Waiting Finance ----------------
                ...
                if user.has_perm("core.view_waiting_finance_sales_deals"):
                    ...
                    return Response(
                        {
                            "detail": "You do not have permission: view_waiting_finance_sales_deals"
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )
            ...
            # ---------------- Entered Finance ----------------
                ...
                if user.has_perm("core.enter_finance_sales_deal"):
                    queryset = queryset.filter(
                        is_entered_in_finance_system="1",
                        form_status="Complete",
                        is_approved_rejected__in=["A", "F"],
                    )
                else:
                    return Response(
                        {
                            "detail": "You do not have permission: enter_finance_sales_deal"
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )
            ...
            # ---------------- Pending Finance ----------------
                ...
                if user.has_perm("core.view_pending_finance_sales_deal"):
                    ...
                    return Response(
                        {
                            "detail": "You do not have permission: view_pending_finance_sales_deal"
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )
            ...
            # ---------------- Draft ----------------
                ...
                if user.has_perm("core.view_my_draft_sales_deals"):
                    ...
                    return Response(
                        {
                            "detail": "You do not have permission: view_my_draft_sales_deals"
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )
            ...
            # ---------------- All ----------------
                ...
                if user.has_perm("core.view_all_sales_deals"):
                    ...
                    return Response(
                        {"detail": "You do not have permission: view_all_sales_deals"},
                        status=status.HTTP_403_FORBIDDEN,
                    )
        ...
        serializer = SalesDealSerializerFordatafilter(
...
SalesDealViewSet_update = SalesDealViewSet.as_view(
    {"put": "update_sales_deal", "get": "update_sales_deal"}
)
Sales_DealViewSet_update_single_field = SalesDealViewSet.as_view(
    {"put": "update_single_field"}
)
SalesDealViewSet_create = SalesDealViewSet.as_view({"post": "create_sale_deal"})
SalesDealViewset_edit_reference_number = SalesDealViewSet.as_view(
...
def create_sales_deal_page(request):
    ...
    sales_data = {"agents": agents, "receipts": receipts}
    print("Agents Data:", agents)

    return render(
        request, "home/createsalesdeal.html", {"sales_data": json.dumps(sales_data)}
    )
...
def edit_sales_deal_page(request, pk):
    ...
    receipts_list_payload = _parse_receipts_list(
        getattr(sales_deal, "receipts_list", "")
    )
    ...
    return render(
        request,
        "home/editsalesdeal.html",
        {
            "deal_id": pk,
            "aws_url": aws_url,
            "reference_number": sales_deal.reference_number,
