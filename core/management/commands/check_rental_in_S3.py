...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        csv_path = f"reports/missing_files_{timestamp}.csv"
        ...
        for rental in query:
            ...
            for field in file_fields:
                ...
                for filename in [f.strip() for f in value.split(",") if f.strip()]:
                    total_checked += 1
                    s3_path = f"rental/referencenumber_CP/{rental.reference_number}/{filename}"
                    ...
                    if found:
                        csv_writer.writerow({
                            "Deal ID": rental.id,
                            "Reference Number": rental.reference_number,
                            "Field": field,
                            "File Name": filename,
                            "S3 Path": s3_path,
                            "Severity": "WARNING" 
